...
# Central Weighted ####
Central_RSC = RSC_Proportions%>%filter(Region=="Alabama"|Region =="Florida")%>%
  group_by(HabitatType)%>%
  summarize(RSC_nfish=sum((Number)))%>%filter(HabitatType!="Total")%>%
  mutate(prop_RSC=RSC_nfish/sum(RSC_nfish))
...
Comps_Final = rbind(Central_lfd_wide,Central_lfdw_wide,east_lfd_wide,west_lfd_wide,west_lfdw_wide)

write.csv(Comps_Final,"GRSC_weightedandNominal_LengthComps.csv")
...
for (i in 1:length(SID.list)){
 # i=3
data = comps_final_long%>%filter(SID==SID.list[i])%>%mutate(lbin=as.numeric(lbin))
...
ggplot(data,aes(x=lbin,y=prop,fill=Type))+
  ...
  labs(x="Fork Length (cm)",y="Proportion at size",fill="Composition Type",
       title= paste0(SID.list[i], " n = ",n.dat))+
  theme_minimal()

Filename=paste0(SID.list[i],"_Final_LFD.jpg")
